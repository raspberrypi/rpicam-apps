...
class RobImageHeader
{
public:
	RobImageHeader( unsigned int width, unsigned int height, unsigned int format)
	{
		_width =width;
		_height = height;
		_format = format;
...
static void writeRaw(std::vector<libcamera::Span<uint8_t>> const &mem, StreamInfo const &info,
			  std::string const &filename)
{
	...
	try
	{
		...
		RobImageHeader file_header( info.width, info.height, 888 );

		if (fwrite((uint8_t *)&file_header, sizeof(file_header), 1, fp) != 1 )
		{
	...
	for (;;)
	{
		...
		if (app.ViewfinderStream())
		{
			...
			if (options->timeout && now - start_time > std::chrono::milliseconds(options->timeout))
			{
				app.StopCamera();
				app.Teardown();
				app.ConfigureStill(LibcameraApp::FLAG_STILL_RGB);
				app.StartCamera();
			}
			else
			{
		...
		else if (app.StillStream())
		{
			...
			//bmp_save(mem, info, options->output, options);
			writeRaw(mem, info, "image.raw");
