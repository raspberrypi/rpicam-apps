...
- **Sliders** for brightness, shutter (logarithmic scale), and framerate (1 fps integer steps), EV, contrast, saturation, sharpness, gain, zoom, AWB gains
- **Dropdowns** for AWB, metering, exposure, denoise, HDR modes
- **Autofocus (AF) controls** are implemented in code but hidden in the UI — untested due to missing hardware (no Raspberry Pi AF-capable lens available); not production-ready
...
- **Shutter auto-resend**: when the framerate slider is adjusted, the shutter µs is recalculated from the new frame period and sent to the camera automatically (if shutter is not on auto) — the displayed value always matches what the camera uses
