...
class CinePIEncoder : public LibcameraApp
{
public:
	using Stream = libcamera::Stream;
	using FrameBuffer = libcamera::FrameBuffer;

	CinePIEncoder() : LibcameraApp(std::make_unique<RawOptions>()) {}

	void StartEncoder()
	{
		createEncoder();
		encoder_->SetInputDoneCallback(std::bind(&CinePIEncoder::encodeBufferDone, this, std::placeholders::_1));
